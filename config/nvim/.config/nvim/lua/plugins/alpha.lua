  ...
  config = function()
    ...
    local function get_recent_files()
      ...
      for i, file in ipairs(oldfiles) do
        if #recent >= max_shown then break end
    ...
    local recent_files = {
      type = "group",
      val = function()
        local header = { type = "text", val = "Recent Files (press r for more)", opts = { hl = "SpecialComment", position = "center" } }
        ...
        for i, file in ipairs(oldfiles) do
          ...
          local button = dashboard.button(tostring(i), display_text, ":e " .. vim.fn.fnameescape(file) .. "<CR>")
    ...
    dashboard.section.buttons.val = {
      make_menu_button("f", "     Find file", ":lua require('telescope.builtin').find_files({hidden=true, no_ignore=false, file_ignore_patterns={'^%.git/', '%.git/'}})<CR>"),
    ...
    local function highlight_alpha_cursor_line()
      ...
      if vim.bo[bufnr].filetype ~= "alpha" then return end
