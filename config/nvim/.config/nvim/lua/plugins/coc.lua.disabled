return {
  ...
  config = function()
    ...
    -- Arrow key navigation for coc completion popup
    keyset("i", "<Down>", 'coc#pum#visible() ? coc#pum#next(1) : "\\<Down>"', opts)
    keyset("i", "<Up>", 'coc#pum#visible() ? coc#pum#prev(1) : "\\<Up>"', opts)

    -- Make <CR> to accept selected completion item with higher priority
    ...
    keyset("i", "<C-f>", 'coc#float#has_scroll() ? "<c-r>=coc#float#scroll(1)<cr>" : "<Right>"', opts3)
    keyset("i", "<C-b>", 'coc#float#has_scroll() ? "<c-r>=coc#float#scroll(0)<cr>" : "<Left>"', opts3)
